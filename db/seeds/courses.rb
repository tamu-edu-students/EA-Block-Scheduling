...
ActiveRecord::Base.transaction do
  ...
  courses.each do |course_data|
    ...
    coreq_string = corequisites.key?(base_code) ? corequisites[base_code].map(&:strip).join(', ') : nil
    type = extract_type(course_data[:sec_name])
    category_string = categories.key?(type) ? categories[type].strip : nil
    Course.find_or_create_by(course_data.merge(prerequisites: prereq_string, corequisites: coreq_string, category: category_string))
  end
