...
roles.each do |role_data|
  Role.find_or_create_by!(role_data)
end

p 'Add test users to Users table'
users = [
  { email: 'test_student@tamu.edu', first_name: 'Test', last_name: 'McTesterson', uid: '123456789012345612345', provider: 'google_oauth2' },
  { email: 'test_admin@tamu.edu', first_name: 'Test', last_name: 'McAdmin', uid: '543219876543211234567', provider: 'google_oauth2' },
]
...
p 'Add test user roles'
user_roles = [
  { user_id: User.find_by(email: 'test_student@tamu.edu').id, role_id: Role.find_by(name: 'student').id },
  { user_id: User.find_by(email: 'test_admin@tamu.edu').id, role_id: Role.find_by(name: 'admin').id }
]

user_roles.each do |ur|
  UserRole.find_or_create_by!(ur)
end
